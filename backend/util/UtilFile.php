...
class UtilFile {
	...
	public static function upload_file($tmp_file_name, $file_name, $file_dir, $parm = 0644) {
		...
		if (is_uploaded_file($tmp_file_name)) {
			if (move_uploaded_file($tmp_file_name,  $file_dir . '/' . $file_name)) {
				return chmod( $file_dir . '/' . $file_name, $parm);
			}else{
				return 1;
			}
		}else{
	 ...
	 * 最新のプロフィール画像名を取得する
	 * 
	 * @param string セッションのプロフィール画像名（tsパラメータ付き）
	 * @return string プロフィール画像名（tsパラメータ付き）
	 */
	public function get_latest_profile_image ($session_image) {

		$session_image_file = explode("?ts=", $session_image)[0];
		$session_image_time = explode("?ts=", $session_image)[1];
		$file_image_time = date("YmdHis", filemtime(RESOURCE_PROFILE_DIR . '/' . $session_image_file));

		if ($session_image_time < $file_image_time)			
			return $session_image_file . '?ts=' . $file_image_time;
