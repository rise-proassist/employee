...
abstract class BaseController {
	 ...
	 * $is_batch 
	 */
	function __construct($is_batch = false) {
	...
	/**
	 * 実行アクションを、同一コントローラ内の別のアクションに切り替える
	 *
	 * @return string アクション名
	 */
	...
	public function run() {
		...
		try {
			...
			// echo $e->getMessage();
			$this->_logger->fatal('[Exception] [ERROR_CODE]:' . $e->getCode() . ' [ERROR_MESSAGE]:' . $e->getMessage());
	...
	protected function initView() {
		...
		$this->_view->template_dir = ADMIN_ROOT_DIR . '/view/';
		$this->_view->compile_dir = ADMIN_ROOT_DIR . '/tmp/';
